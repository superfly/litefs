...
func (c *Client) Begin(ctx context.Context, primaryURL string, nodeID, name string) (_ litefs.RemoteTx, retErr error) {
	...
	*u = url.URL{
	...
	remoteTx := &RemoteTx{w: pw, r: resp.Body}
	defer func() {
		if retErr != nil {
			_ = remoteTx.Rollback()
		}
	...
	if err := binary.Read(resp.Body, binary.BigEndian, &remoteTx.id); err != nil {
		return nil, fmt.Errorf("read txid: %w", err)
	}
	if err := binary.Read(resp.Body, binary.BigEndian, &remoteTx.preApplyChecksum); err != nil {

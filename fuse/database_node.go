...
func (n *DatabaseNode) lock(ctx context.Context, req *fuse.LockRequest) error {
	...
	if len(lockTypes) == 0 {
		log.Printf("dbg/LOCK.err.nolocks")
		return fmt.Errorf("no locks")
	} else if len(lockTypes) > 1 {
		log.Printf("dbg/LOCK.err.multiple")
	...
	mu, guard, name, err := n.mutexAndGuardRefByLockType(req.LockOwner, lockType)
	if err != nil {
		log.Printf("dbg/LOCK.err.magrblt")
		return err
	}
	...
	if *guard != nil {
		...
		case fuse.LockRead:
			log.Printf("dbg/LOCK.guard.read %q", name)
			(*guard).RLock()
			return nil
		case fuse.LockWrite:
			if !(*guard).TryLock() {
				log.Printf("dbg/LOCK.guard.trylock.fail %q", name)
				return fuse.Errno(syscall.EAGAIN)
			}
			log.Printf("dbg/LOCK.guard.trylock.OK %q", name)
			return nil
		default:
	...
	case fuse.LockRead:
		if *guard = mu.TryRLock(); *guard == nil {
			log.Printf("dbg/LOCK.mutex.tryrlock.fail %q", name)
			return fuse.Errno(syscall.EAGAIN)
		}
		log.Printf("dbg/LOCK.mutex.tryrlock.OK %q", name)
		return nil
	case fuse.LockWrite:
		if *guard = mu.TryLock(); *guard == nil {
			log.Printf("dbg/LOCK.mutex.trylock.fail %q", name)
			return fuse.Errno(syscall.EAGAIN)
		}
		log.Printf("dbg/LOCK.mutex.trylock.OK %q", name)
		return nil
	default:
		panic(fmt.Sprintf("invalid posix lock type: %d", typ))
	}
...
func (n *DatabaseNode) unlock(ctx context.Context, req *fuse.UnlockRequest) error {
	...
	for _, lockType := range lockTypes {
		_, guard, name, err := n.mutexAndGuardRefByLockType(req.LockOwner, lockType)
		if err != nil {
			log.Printf("dbg/UNLOCK.err %q", name)
			return err
		} else if *guard == nil {
			log.Printf("dbg/UNLOCK.noguard.skip %q", name)
			continue // no lock acquired, skip
		}

		// Unlock and drop reference to the guard.
		log.Printf("dbg/UNLOCK %q", name)
		(*guard).Unlock()
		*guard = nil
	}
	return nil
...
func (n *DatabaseNode) canLock(owner fuse.LockOwner, typ fuse.LockType, lockType litefs.LockType) (bool, error) {
	mu, guard, name, err := n.mutexAndGuardRefByLockType(owner, lockType)
	if err != nil {
		return false, err
	}
	log.Printf("dbg/CANLOCK guard=%v %q", *guard != nil, name)
